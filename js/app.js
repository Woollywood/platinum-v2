...
*/    var _suppressOverwrites, _reverting, _context, _globalTimeline, _win, _coreInitted, _doc, _coreReady, _lastRenderedFrame, _quickTween, _tickerActive, _config = {
    ...
    Tween.version = Timeline.version = gsap_core_gsap.version = "3.12.1";
    _coreReady = 1;
    _windowExists() && _wake();
...
*/    var getOwnPropertySymbols = Object.getOwnPropertySymbols;
    ...
    function getTargetElements(target) {
        ...
        return dist_toArray(elements).reduce(((result, element) => [ ...result, ...dist_toArray(element).filter(isNode) ]), []);
    }
    ...
    function createElement(name, attributes) {
        ...
        Object.keys(attributes).forEach((attribute => {
            ...
            if (attribute === "children") element.append(...dist_toArray(value)); else element.setAttribute(attribute, value);
        }));
        return element;
    }
    var dist_defaults = {
    ...
    function splitWordsAndChars(textNode, settings) {
        settings = extend(dist_defaults, settings);
    ...
    function split(node, settings) {
        ...
        const childNodes = dist_toArray(node.childNodes);
    ...
    function unSplitWords(element) {
        if (!get(element).isWord) dist_toArray(element.children).forEach((child => unSplitWords(child))); else {
            remove(element);
    ...
    function repositionAfterSplit(element, settings, scrollPos) {
        ...
        dist_toArray(nodes).forEach((node => {
    ...
    let split_type_dist_defaults = extend(dist_defaults, {});
    class SplitType {
        static get data() {
            return cache;
        }
        ...
        static set defaults(options) {
            split_type_dist_defaults = extend(split_type_dist_defaults, parseSettings(options));
        }
        ...
        constructor(elements, options) {
            this.isSplit = false;
            this.settings = extend(split_type_dist_defaults, parseSettings(options));
    ...
    class Slider {
        ...
        _breakpointsInit() {
            if (this._options.breakpoints) this._breakpointsOption = new Map(Object.entries(this._options.breakpoints));
        }
        _readBreakpoints() {
            for (const [breakpoint, option] of Object.entries(this._options.breakpoints).reverse()) if (window.innerWidth >= breakpoint) {
        ...
        _getSlidesWidth() {
            ...
            this._slides.forEach(((slide, index) => {
                ...
                if (index < this._slides.length - 1) slide.style.marginRight = `${this._spaceBetween}px`;
            }));
        }
    }
    class SliderAnimation {
        ...
        _init() {
            this._animationTimeline = gsapWithCSS.timeline({
                repeat: -1,
                repeatDelay: 2,
                yoyo: true
            });
            const slides = this._sliderInstance._slides;
            const fullWidth = Array.from(slides).reduce(((result, slide, index) => result += slide.getBoundingClientRect().width + (index < slides.length - 1 ? this._sliderInstance._spaceBetween : 0)), 0);
